...
- Gemini raw API errors must not be shown in the default user screen. Show a short friendly notice; keep raw details only in a developer expander.
- User-facing screens should avoid developer terms such as `proxy`, `style_vector`, `pgvector`, `attributes_jsonb`, and `10x10 Grid`; if needed, place them in developer-only expanders.
- Scouting Board should default to players with an ability profile. Basic-info/Limited candidates belong behind advanced controls.
- Style & Mentor Lab should be mentor-first: age-filtered mentor candidates are the main surface, while similarity references stay hidden by default.
- Descriptive Career Simulation controls must map back to the existing numeric environment settings; Growth/Ceiling formulas remain unchanged.

## v19 Final Folder Organization: docs archive and root cleanup

- `docs/specs/`, `docs/tasks/`, `docs/reports/`, `docs/archive/` 폴더 구조 생성.
- 완료된 task 문서(10개)는 `docs/tasks/`, spec(1개)은 `docs/specs/`, report(2개)는 `docs/reports/`, archive(4개)는 `docs/archive/`로 이동.
- `CLAUDE_PROGRESS_SUMMARY_ARCHIVE.md` → `docs/archive/CLAUDE_PROGRESS_SUMMARY_ARCHIVE.md`로 이동. 관련 경로 참조 업데이트.
- `DB_HELPER_DIFF.md` → `docs/reports/DB_HELPER_DIFF.md`로 이동. ACTIVE_FUNCTION_MAP.md 경로 참조 업데이트.
- 실제 import되는 Python 파일(`evidence_extractor.py`, `grid_pipeline.py` 포함)은 루트에 유지.
- 테스트 파일은 루트에 유지.
- 삭제 후보: `ui/` 폴더(현재 미사용), `archive/backups/`, `__pycache__` — 이번 작업에서 삭제하지 않음.
- `FOLDER_ORGANIZATION_REPORT.md` 신규 생성.

## v19 Final Polish and Cleanup

- `services/db.py`: `get_distinct_nationalities()`, `get_distinct_clubs()` 추가.
- `ui_components.py`: `POSITION_OPTIONS`, `FOOT_OPTIONS`, `build_position_options()`, `build_nationality_options()`, `build_club_options()` 추가.
- `views/manual_prospect.py`: position/sub_position/nationality/club/foot 입력이 모두 selectbox로 전환. "기타 / 직접 입력" fallback 패턴 적용.
- `explanation_engine.py`: `FEATURE_SOURCE_BADGES`, `build_strengths_with_meta()`, `build_risks_with_meta()` 추가. `explain_feature_score()`, `build_recommendations()` 4-part 구조로 확장.
- `views/dashboard.py`: `_render_strength_risk_panels()` 헬퍼 추가. 강점/리스크/추천 패널에 소스 배지 표시.
- `views/career_simulation.py`: 코칭 시나리오 패널에 Ceiling Scenario / Growth Model / Rule-based 배지 + 관찰 지표 섹션 추가.
- `views/ai_report.py`: 8섹션 리포트 구조 + FM proxy / Rule-based / style_vector 소스 라벨 추가. 섹션 키 `"Growth Model Insight"`, `"Ceiling Scenario Insight"`는 테스트 호환을 위해 유지.
- `analysis_helpers.py`: `generate_similarity_reason()`, `generate_mentor_guide()` — FM proxy / style_vector 출처 라벨 + 구조화 텍스트 적용.
- 주의: `explanation_engine.py` contribution_score 추천에 "판단 속도" 키워드 유지 (테스트 호환).
- `CLEANUP_AUDIT_REPORT.md` 생성.

## v19 UI Redesign Phase C: Style & Mentor Lab and Scouting Notes styling

- `views/legend_matching.py`와 `views/scouting_notes.py`가 Phase A/B와 동일한 dark sports management UI로 개편되었다.
- `components/cards.py`에 mentor_card_html, similarity_card_html, archive_note_card_html, empty_state_panel_html, score_bar_html이 추가되었다.
- `styles/game_ui.css`에 Phase C 전용 CSS 클래스(.game-mentor-card, .game-similarity-grid, .game-note-card, .game-archive-strip 등)가 추가되었다.
- pgvector style_vector gating과 mentor age filter 로직은 변경하지 않는다.
- Notes 저장 payload, legacy fallback, saved_note_label/saved_coaching_sections/saved_report_original public 함수는 삭제하지 않는다.
- Manual Prospect, DB Status는 아직 전체 개편하지 않는다.

## v19 UI Redesign Phase A: Game-style shell, Scouting Board, and Player Dossier

- `components/` 구조가 추가되었다: `layout.py`, `cards.py`, `badges.py`, `player_header.py`, `attribute_panels.py`.
- `styles/` 구조가 추가되었다: `theme.py`, `game_ui.css`.
- 공통 dark sports management UI는 `styles/theme.py::load_game_ui_css()`와 `styles/game_ui.css`에서 관리한다.
- `theme.py::apply_theme()`는 새 CSS loader를 호출한다.
- `app.py`는 라우터 중심 파일로 유지하고, 대형 HTML/CSS를 넣지 않는다. 사이드바 브랜드처럼 필요한 shell 호출만 얇게 둔다.
- Scouting Board와 Player Dossier에는 Phase A UI가 우선 적용되었다.
- Style & Mentor Lab, Career Simulation, Evidence & Advisory Report, My Scouting Notes, Manual Prospect, DB Status 전체 개편은 아직 하지 않는다.
- 새 UI 작업 중에도 DB 스키마, secrets/env, 원본 CSV, Growth/Ceiling 공식, Gemini 역할, Notes 저장 구조를 변경하지 않는다.
- Football Manager 로고, 이미지, 고유 에셋, 고유 UI를 복제하지 않고 독자적인 dark navy scouting UI만 사용한다.
...
## v19 UI Redesign Phase A.1 Handoff

- Shared UI CSS should live in `styles/game_ui.css`; keep root `theme.py` as a thin loader and avoid growing `app.py` for view-specific UI.
- Phase A.1 polished only Scouting Board and Player Dossier. Do not assume Style & Mentor Lab, Career Simulation, Evidence & Advisory Report, My Scouting Notes, Manual Prospect, or DB Status have been fully redesigned.
- `docs/archive/CLAUDE_PROGRESS_SUMMARY_ARCHIVE.md` was intentionally not read or modified during Phase A.1.
## v19 UI Redesign Phase B Handoff

- Phase B applies the shared game-style design system to Career Simulation and Evidence & Advisory Report only.
- Career Simulation UI changes must not alter Growth/Ceiling formulas, scenario adjustment range, `ceiling_growth_*` session_state keys, or Notes save payloads.
- Evidence & Advisory Report UI changes must keep Gemini limited to qualitative signal extraction and advisory text. Gemini must not calculate scores or invent unsupported facts.
- Style & Mentor Lab, My Scouting Notes, Manual Prospect, and DB Status are still not fully redesigned.
